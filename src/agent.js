...
function systemPrompt(context) {
  ...
  const lines = [
    ...
    "• Always use *bold* (single asterisk each side) for the key term or topic name in every answer — e.g. *Roti* is a flatbread...",
    "• For any definition or explanation (What is X, Who is Y, Tell me about Z): bold the topic name, then give 1–2 sentence intro, then a blank line, then numbered or bulleted sub-points (origin, ingredients, uses, etc.), then a blank line, then a 1-sentence closing.",
    ...
    "• Leave ONE blank line between every paragraph, after every heading, and between list items — never run text together.",
    "• *Bold* section labels like *Ingredients:*, *Origin:*, *Uses:* on their own line before their content.",
    "• Never write a plain wall of text — every answer must have visible structure with bold, spacing, or lists.",
